...
for k, v in {
    ...
    "start_time": None,
    "alerts": 0,
    "history_log": [],      # [{ts, filename, focused, f_pct, d_pct}]
    "saved_sessions": []    # [{ts, log, duration, alerts, score}]
}.items():
...
.sb-brand {
    font-size: 1.5rem; font-weight: 700; color: #E8F0FF;
...
.sb-section {
    font-size: 10px; color: #7A9CBF; text-transform: uppercase;
    letter-spacing: .12em; margin: 10px 4px 6px 4px; font-weight: 600;
}
...
.teal-dot {
    width: 7px; height: 7px; background: #2DD4BF;
...
.stat-lbl {
    font-size: 10px; color: #7A9CBF; margin-top: 2px;
    text-transform: uppercase; letter-spacing: .05em;
}

/* ── Empty states ── */
.empty-result {
    border: 1.5px dashed #1E3050; border-radius: 12px;
    padding: 4rem; text-align: center; color: #7A9CBF;
    font-size: 13px; background: #080F1E;
}
.live-empty {
    border: 1.5px dashed #1E3050; border-radius: 10px;
...
.score-lbl  {
    font-size: 11px; color: #7A9CBF; text-transform: uppercase;
...
.hist-file     {
    font-size: 12px; color: #A8C4DC; flex: 1;
...
/* ── About hero ── */
.about-hero {
    background: linear-gradient(135deg, #112240 0%, #080F1E 100%);
    border: 1px solid #1E3050; border-radius: 16px;
    padding: 2rem; margin-bottom: 1.5rem;
}
.about-hero-title {
    font-size: 2rem; font-weight: 700; color: #E8F0FF;
...
.about-stat-lbl {
    font-size: 10px; color: #7A9CBF; margin-top: 4px;
...
.step-num {
    background: #2DD4BF; color: #06101F; border-radius: 50%;
...
/* ── Creator card ── */
.creator-card {
    background: linear-gradient(135deg, #112240, #080F1E);
    border: 1px solid #1E3050; border-radius: 14px; padding: 1.5rem;
...
/* ── Footer ── */
.footer {
    background: #080F1E; border-top: 1px solid #1E3050;
...
with main_col:
        ...
        with col1:
            ...
            if uploaded:
                ...
                <div class="upload-hint">
                  ...
                  <div style="font-weight:600;color:#A8C4DC;margin-bottom:4px">
                    Click to browse
                  </div>
            ...
            if st.session_state.running:
                ...
                for _ in range(300):
                    ...
                    cv2.putText(frame_lm, f"FaceFlow  |  {label}",
                                (12, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.65, color, 2)
                    cv2.putText(frame_lm, f"{m:02d}:{s:02d}",
                                (w - 70, 32), cv2.FONT_HERSHEY_SIMPLEX,
                                0.65, (74, 96, 128), 1)

                    frame_box.image(cv2.cvtColor(frame_lm, cv2.COLOR_BGR2RGB),
                                    use_container_width=True)
                    time.sleep(0.05)

                cap.release()

                # Save session automatically when loop ends
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        if not sessions and not hist:
            ...
            <div class="empty-result">
              ...
              <div style="font-weight:600;color:#A8C4DC;margin-bottom:6px">No reports yet</div>
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        if not hist:
            ...
            <div class="empty-result">
              <div style="font-size:2.5rem;margin-bottom:10px">🕐</div>
              <div style="font-weight:600;color:#A8C4DC;margin-bottom:6px">No history yet</div>
              <div style="font-size:12px;color:#7A9CBF">
                Go to <b>Photo Analysis</b>, upload an image and click Analyze.
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        <div class="about-hero">
          ...
          <div class="about-hero-sub">
            FaceFlow is a real-time AI-powered focus detection system that uses computer
            vision and machine learning to determine whether a student or professional is
            mentally engaged or distracted. It detects focus state through subtle eye and
            facial cues — providing instant feedback and detailed session analytics to help
            you stay productive. No wearables, no special hardware. Just your webcam.
          </div>
        ...
        <div class="about-grid">
          <div class="about-card">
            ...
            <div class="about-b">Students and professionals lose focus without
              realizing it. Traditional productivity tracking is manual and inaccurate.
              FaceFlow detects the exact moment attention drifts using passive,
              non-intrusive computer vision.</div>
          </div>
          <div class="about-card">
            ...
            <div class="about-b">MediaPipe maps 468 facial landmarks per frame.
              ...
              trained on 84,000+ labeled eye images. The model outputs a
              focused / distracted prediction with confidence scores.</div>
          </div>
          <div class="about-card">
            <div class="about-icon">📊</div>
            <div class="about-t">Session Analytics</div>
            <div class="about-b">Every session produces a focus score, a timeline
              bar chart, and a focus / distraction pie chart. All analyses are logged
              in History and full PNG reports can be exported for personal
              productivity tracking.</div>
          </div>
          <div class="about-card">
            <div class="about-icon">📸</div>
            <div class="about-t">Photo Mode</div>
            <div class="about-b">Upload any front-facing photo to instantly detect
              the focus state. The system overlays MediaPipe face mesh landmarks and
              shows detailed confidence scores. Results are auto-logged to History.</div>
          </div>
          <div class="about-card">
            <div class="about-icon">🎥</div>
            <div class="about-t">Live Webcam Mode</div>
            <div class="about-b">Real-time focus detection at ~20 FPS using your
              webcam. A live HUD displays focus state and session timer. Distraction
              alerts are counted and the full session timeline is saved to Reports
              automatically when you stop.</div>
          </div>
          <div class="about-card">
            <div class="about-icon">🔒</div>
            <div class="about-t">Privacy First</div>
            <div class="about-b">All processing happens 100% locally on your device.
              No images, video, or personal data are ever sent to any server.
              FaceFlow works entirely offline after installation — your face data
        ...
        <div class="pipeline-step">
          ...
          <div>
            <div class="step-title">Frame Capture</div>
            <div class="step-desc">Live video is captured via OpenCV's VideoCapture API.
              Each frame is flipped horizontally for a natural mirror view and passed
              to the landmark detector.</div>
          </div>
        </div>
        <div class="pipeline-step">
          <div class="step-num">2</div>
          <div>
            <div class="step-title">Face Mesh Detection (MediaPipe)</div>
            <div class="step-desc">MediaPipe FaceMesh detects 468 3D facial landmarks.
              Key eye landmarks (indices 33, 263, 159, 386) are used to precisely crop
              the bilateral eye region from the frame with 10–20 px margin.</div>
          </div>
        </div>
        <div class="pipeline-step">
          ...
          <div>
            ...
            <div class="step-desc">The cropped eye region is resized to 64×32 px and
              converted to grayscale. Six statistical features are extracted: mean
              brightness, upper/lower split brightness, standard deviation, bright
              pixel count (&gt;150), and dark pixel count (&lt;80).</div>
          </div>
        </div>
        <div class="pipeline-step">
          <div class="step-num">4</div>
          <div>
            <div class="step-title">Random Forest Classification</div>
            <div class="step-desc">The 6-feature vector is passed to a trained Random
              Forest model (focus_model.pkl). The model outputs a binary prediction
              (focused = 1, distracted = 0) plus class probabilities used as
              confidence scores shown in the UI.</div>
          </div>
        </div>
        <div class="pipeline-step">
          <div class="step-num">5</div>
          <div>
            <div class="step-title">Result Display &amp; Logging</div>
            <div class="step-desc">Results are displayed with confidence bars and
              landmark overlays. Each analysis is appended to the History log, and
              webcam sessions are saved to Reports with auto-generated charts.</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="tech-grid">
          <div class="tech-card">
            ...
            <div>
              ...
              <div class="tech-desc">Core language — clean, expressive, ecosystem-rich</div>
            </div>
          </div>
          <div class="tech-card">
            <div class="tech-icon">🤖</div>
            <div>
              ...
              <div class="tech-desc">Google's real-time 468-point face mesh detection</div>
            </div>
          </div>
          <div class="tech-card">
            <div class="tech-icon">📷</div>
            <div>
              ...
              <div class="tech-desc">Video capture, image processing &amp; frame annotation</div>
            </div>
          </div>
          <div class="tech-card">
            <div class="tech-icon">🌲</div>
            <div>
              ...
              <div class="tech-desc">Random Forest classifier trained on 84K+ eye images</div>
            </div>
          </div>
          <div class="tech-card">
            <div class="tech-icon">🔢</div>
            <div>
              ...
              <div class="tech-desc">Fast numerical computation for feature extraction</div>
            </div>
          </div>
          <div class="tech-card">
            <div class="tech-icon">🌊</div>
            <div>
              ...
              <div class="tech-desc">Interactive web UI with real-time webcam streaming</div>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="creator-card">
          ...
          <div>
            ...
            <div class="creator-bio">
              Built FaceFlow from scratch over 4 weeks as a passion project exploring
              real-world computer vision. The system demonstrates a full end-to-end ML
              pipeline: dataset curation, model training, feature engineering, and
              production deployment via Streamlit. Hamna is a Software Engineering
              student from Pakistan specializing in AI/ML systems.
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
